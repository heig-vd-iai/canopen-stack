...
class CustomNode(canopen.Node):
    def __init__(self, node_id: int, object_dictionary: str, load_od: bool = False):
        ...
        for i in self.tpdo:
            self.tpdo[i].add_callback(self._on_TPDO)
        self.emcy.add_callback(self._on_EMCY)

    def _on_TPDO(self, message):
        print(f"{message.name}:")
        for var in message:
            print(f"{var.name} : {var.phys}")

    def _on_EMCY(self, emcy):
        print(f"Emergency code 0x{emcy.code:04X}: {emcy.get_desc()}")

    def print_errorRegister(self):
        print(f"[1001][Error register]: {self.sdo[0x1001].phys:#010b}")

    def print_errorField(self):
        print(
            f"[1003][Pre-defined error field]: {', '.join([f'0x{var.phys:X}' for var in self.sdo[0x1003].values()])}"
        )

    def saveParameters(self):
        self.sdo[0x1010][1].raw = "save".encode()

    def restoreParameters(self):
        self.sdo[0x1011][1].raw = "load".encode()

    def setHeartbeatTime(self, value_ms: int):
        self.sdo[0x1017].phys = value_ms


node = CustomNode(1, "../tests/golden/minimal/od.eds")
...
try:
    ...
    # with node.sdo[0x2002].open("wb", size=len(data), block_transfer=True) as f:
    #     f.write(data)

    # Block upload
    # with node.sdo[0x2002].open("rb") as f:
    ...
    node.tpdo[i].add_variable(0x6041, 0)
    node.tpdo[i].add_variable(0x6064, 0)
    node.tpdo.save()
