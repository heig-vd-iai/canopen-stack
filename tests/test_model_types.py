...
from generator.validation.models.types import ObjectId, ObjectType
...
def test_object_type_creation():
    """Test successful creation of ObjectType."""
    obj_type = ObjectType("var")
    assert obj_type.name == "var"
    assert obj_type.code == 0x7


def test_object_type_invalid():
    """Test that invalid object type raises ValueError."""
    with pytest.raises(ValueError, match="Invalid object type name: foo"):
        ObjectType("foo")
