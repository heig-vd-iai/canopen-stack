...
class Object1A00(RecordObject):
    ...
    def __init__(self, index: int, entries: "list[Variable]", entriesGranularity: int = 1) -> None:
        super().__init__(index, entries, "Object1A00", entriesGranularity)
        self._MAX_SIZE = 8
    ...
    def verify(self, objects: dict) -> bool:
        ...
        for i in range(self.entries[0].value):
            ...
            if index not in objects:
                self.error(
                    f"invalid TPDO mapping for sub {i + 1} (invalid index)")
                retval = False
            elif objects[index].subNumber <= subindex:
                self.error(
                    f"invalid TPDO mapping for sub {i + 1} (invalid subindex)")
                retval = False
