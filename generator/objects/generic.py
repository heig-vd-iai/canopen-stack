    ...
    def __init__(self, index: int, entries: "list[Variable]", cppObjectName: str = "Object", cppBaseEntryName: str = "ObjectEntryBase", entriesGranularity: int = 1) -> None:
        ...
        for i, entry in enumerate(entries):
            ...
            if entry.data_type not in datatype2entryclass:
                self.error(
                    f"unsupported data type '{entry.data_type:X}' for sub {i}")
                entryValid = False
            if entry.access_type not in accesstypes:
                self.error(
                    f"unknown access type '{entry.access_type}' for sub {i}")
                entryValid = False
            if (entry.min is None) != (entry.max is None):
                self.error(
                    f"LowLimit and HighLimit must be both declared or not at all for sub {i}")
                entryValid = False
            if entryValid:
        ...
        for i, entry in enumerate(self.entries):
            if entry.size <= 0:
                self.error(
                    f"invalid size '{entry.size}' for sub {i}, must be greater than 0")
                errors = True
            if entry.size > ENTRY_MAX_SIZE:
                self.error(
                    f"invalid size '{entry.size}' for sub {i}, must be lower than {ENTRY_MAX_SIZE}")
                errors = True
        if errors:
            raise ValueError()
...
class ArrayObject(ObjectBase):
    ...
    def renderEntry(self) -> "list[str]":
        ...
        subs = [entry.renderEntry(f"{self.varName}sub{sub + 1}", f"{self.varName}[{sub}]")
                for sub, entry in enumerate(self.entries[1:])]
        return [sub0, *subs]
...
class RecordObject(ObjectBase):
    ...
    def renderData(self) -> "list[str]":
        ...
        fields = "; ".join(
            [entry.renderData(f"sub{i}") for i, entry in enumerate(self.entries)]) + ";"
