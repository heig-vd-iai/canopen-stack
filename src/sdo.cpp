...
//    uint32_t bytesSent = transferData.object->getSize(transferData.subindex) -
//                         transferData.remainingBytes;
...
//    uint32_t abortCode;
//    if ((abortCode = transferData.object->readBytes(
//             transferData.subindex, response.data + SDO_SEGMENT_DATA_OFFSET,
...
//                 transferData.remainingBytes, node)) != SDOAbortCode_OK) {
...
//    (recvCommand.bits_downClientInitiate.cs !=
...
//        transferData.ackseq++;
//        uint32_t payloadSize =
//            transferData.remainingBytes > SDO_BLOCK_DATA_LENGTH
//                ? SDO_BLOCK_DATA_LENGTH
//                : transferData.remainingBytes;
...
//    }
//    if (++transferData.seqno >= transferData.blksize ||
//        recvCommand.bits_downClientSub
//            .c) {  // Last sub-block, send confirmation to client
...
//                           uint32_t timestamp_us) {
//       SDOBlockCommandByte sendCommand = {0}, recvCommand =
//       {request.getCommandByte()};
//       (recvCommand.bits_downClientEnd.cs != SDOSubCommand_ClientDownloadEnd)
//       {
//           sendAbort(SDOAbortCode_CommandSpecifierInvalid);
//           return;
//       }
//       uint32_t abortCode;
//       if ((abortCode =
//       transferData.object->writeBytes(transferData.subindex, buffer.data,
...
//            if (!transferData.object->getMetadata(transferData.subindex)
//                     .bits.updateFlag)
//                uploadInitiateSend(timestamp_us);
//            break;
