...
SDOAbortCodes Object::preReadBytes(uint8_t /*subindex*/, uint8_t * /*bytes*/,
                                   uint32_t /*sizeBytes*/,
                                   uint32_t /*offset*/) {
    return SDOAbortCode_OK;
}

void Object::postReadBytes(uint8_t /*subindex*/, uint8_t * /*bytes*/,
                           uint32_t /*sizeBytes*/, uint32_t /*offset*/) {}

SDOAbortCodes Object::preWriteBytes(uint8_t /*subindex*/, uint8_t * /*bytes*/,
                                    uint32_t /*sizeBytes*/,
                                    class Node & /*node*/) {
    return SDOAbortCode_OK;
}

void Object::postWriteBytes(uint8_t /*subindex*/, uint8_t * /*bytes*/,
                            uint32_t /*sizeBytes*/, class Node & /*node*/) {}

SDOAbortCodes Object::writeBytes(uint8_t subindex, uint8_t *bytes,
                                 uint32_t sizeBytes, Node &node) {
    if (!isSubValid(subindex)) return SDOAbortCode_SubindexNonExistent;
...
//        return SDOAbortCode_AttemptWriteOnReadOnly;
    ...
    if (limits < 0) return SDOAbortCode_DownloadValueTooLow;
    if (limits > 0) return SDOAbortCode_DownloadValueTooHigh;
    ...
    switch (code) {
        case SDOAbortCode_OK:
            ...
            if (onWriteFunc) onWriteFunc(*this, subindex);
            return SDOAbortCode_OK;
...
void Object::requestUpdate(uint8_t subindex) {
    if (!isRemote() || !isSubValid(subindex)) return;
    //    entries[subindex]->metaData.bits.updateFlag = true; //TODO: change
    onRequestUpdateFunc(*this, subindex);
}

SDOAbortCodes Object::readBytes(uint8_t subindex, uint8_t *bytes,
                                uint32_t sizeBytes, uint32_t offset) {
    if (!isSubValid(subindex)) return SDOAbortCode_SubindexNonExistent;
    ObjectEntryBase *entry = (ObjectEntryBase *)entries[subindex];
...
bool Object::isSubValid(uint8_t subindex) const { return subindex < subNumber; }
...
//    //    return isSubValid(subindex) ? entries[subindex]->metaData :
//    //    MetaBitfield{0};
//}
...
bool Object::isRemote() const { return (bool)onRequestUpdateFunc; }
...
bool Object::setBytes(uint8_t subindex, unsigned bufferSize, uint8_t *buffer) {
    if (!isSubValid(subindex) || bufferSize < entries[subindex]->sizeBytes)
        return false;
    memcpy((void *)entries[subindex]->dataSrc, buffer,
           entries[subindex]->sizeBytes);
