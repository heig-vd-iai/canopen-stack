...
uint8_t ObjectDictionnary::getLocalData(Data &data, uint16_t id, SDOAbortCodes &abortCode){
    data = dataTable[id];
    return 0;
}

uint8_t ObjectDictionnary::setLocalData(Data data, uint16_t id, SDOAbortCodes &abortCode){
    dataTable[id] = data;
    return 0;
}
