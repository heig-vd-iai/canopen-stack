        ...
        for object in self.objects:
            if "enum" in object.prout:
                enums.append(
                    (
                        (object.index, 0),
                        object.prout["enum"]["class"],
                        object.prout["enum"]["data"],
                    )
                )
            for sub in object.get_subobjects:
                if "enum" in sub.data.data:
                    enums.append(
                        (
                            (sub.index, sub.subindex),
                            sub.data.data["enum"]["class"],
                            sub.data.data["enum"]["data"],
                        )
                    )
        ...
        for object in self.objects:
            if object.name == "Modes of operation":
                ...
                for item in items:
                    ...
                    mode_suffix = "".join(
                        word.capitalize() for word in right.split("_")
                    )
    ...
    def to_remote(self):
        ...
        for object in self.objects:
            for subobject in object.get_subobjects:
                if (
                    subobject.data.access in ["ro", "rw"]
                    and subobject.data.data.get("attribute", None)
                    or subobject.get
                ) != "none" and object.remote:
                    prout.append(
                        (
                            subobject.index << 8 | subobject.subindex,
                            (subobject.cpp_instance_name, subobject.type_code),
                        )
                    )
